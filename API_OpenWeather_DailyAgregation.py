...
def OpenWeather_API_DailyAgregation(pCity_name, pCountry_code, pDate):

    # Géolocalisation de la ville
    (latitude, longitude)=OpenWeather_API_Geolocal(pCity_name, pCountry_code)

    # REQUETE
    req="https://api.openweathermap.org/data/3.0/onecall/day_summary?lat="+latitude+"&lon="+longitude+"&date="+pDate+"&appid="+ApiKey+"&units=metric"
...
date=input("Date (YYYY-MM-JJ): ")

# Current weather
weather_infos=OpenWeather_API_DailyAgregation(city_name,country_code,date)
