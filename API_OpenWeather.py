...
# ------------------------------------------------------------
def API_geolocal(pCity_name, pCountry_code='fr'):
...
# ------------------------------------------------------------------
def API_currentWeather_FREE(pCity_name, pCountry_code, pLatitude="#", pLongitude="#"):

    # Géolocalisation de la ville
    if pLatitude=="#":
        (pLatitude, pLongitude)=API_geolocal(pCity_name, pCountry_code)

    # REQUETE
    req=api_url+"/data/2.5/weather?lat="+pLatitude+"&lon="+pLongitude+"&appid="+api_key+"&units=metric"
...
# -------------------------------------------------------------
def API_currentWeather(pCity_name, pCountry_code, pLatitude="#", pLongitude="#"):
    ...
    # REQUETE
    req="https://api.openweathermap.org/data/3.0/onecall?lat="+pLatitude+"&lon="+pLongitude+"&exclude=minutely,hourly,daily,alerts"+"&appid="+api_key+"&units=metric"
...
# -----------------------------------------------------------
def API_dailyAgregation(pCity_name, pCountry_code, pDate, pLatitude="#", pLongitude="#"):
    ...
    # REQUETE
    req=api_url+"/data/3.0/onecall/day_summary?lat="+pLatitude+"&lon="+pLongitude+"&date="+pDate+"&appid="+api_key+"&units=metric"
...
# Interrogation de l'API OpenWeather: infos météo instantannées
# -------------------------------------------------------------
def API_weatherTimeStamp(pCity_name, pCountry_code, pDate, pHour, pLatitude="#", pLongitude="#"):
    ...
    # REQUETE
    req=api_url+"/data/3.0/onecall/timemachine?lat="+pLatitude+"&lon="+pLongitude+"&dt="+timestamp+"&appid="+api_key+"&units=metric"
...
def station_choix_saisie():
    ...
    # Géolocalisation de la ville
    (latitude, longitude)=API_geolocal(city_name, country_code)
...
# Choix de la date
# ----------------
def date_choix_saisie():
    
    print("Choisissez la date:")
...
# Choix de l'heure
# ----------------
def heure_choix_saisie():
    
    print("Choisissez l'heure:")
    hour=input("Heure:")
...
# -------------------------------
def wind_orientation(pwind_dir):
...
# ---------------------------------------
def wind_direction_test():

    print("Direction du vent en degrés ?")
    wind_dir=input("Vent direction:")
    (wind_orient)=wind_orientation(wind_dir)
    print(wind_orient)

def wind_direction_evaluation():
    for i in range(0,361):
        print(i,wind_orientation(i))


# TEST de l'API Current weather FREE
# ----------------------------------
def API_currentWeather_FREE_test():
    print("\n 0. Quel temps fait-il (FREE)?")
    weather_infos=API_currentWeather_FREE(city_name,country_code)
    print("\n","Current weather on "+date+" at ",city_name+"("+country_code+")","\n","\n",weather_infos)
    

# TEST de l'API Current weather
# =============================    
def API_currentWeather_test():
    print("\n 1. Quel temps fait-il?")
    weather_infos=API_currentWeather(city_name,country_code)
    print("\n","Current weather on "+date+" at ",city_name+"("+country_code+")","\n","\n",weather_infos)


# TEST de l'API DailyWeather
# --------------------------
def API_dailyAgregation_test():
    print("\n 2. Quel temps faisait-il la journée du ...?")
    date=input("Date (YYYYMMJJ): ")
    date=date[0:4]+"-"+date[4:6]+"-"+date[6:8]
    weather_infos=API_dailyAgregation(city_name,country_code,date)
    print("\n","Current weather on "+date+" at ",city_name+"("+country_code+")","\n","\n",weather_infos)
...
# ------------------------------
def API_weatherTimeStamp_test():
#    print("\n 3. Quel temps faisait-il le ... à ... heure?")
#    date=input("Date (YYYYMMJJ): ")
#    hour=input("Hour (HHMM):")
            
    weather_infos=API_weatherTimeStamp(city_name,country_code,date,hour)
